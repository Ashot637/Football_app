...
export const fetchLogin = createAsyncThunk(
  ...
  async (params, thunkAPI) => {
    try {
      ...
      console.log("Login response, expoPushToken:", data.expoPushToken); 
      if (data.expoPushToken) {
        thunkAPI.dispatch(setExpoPushToken(data.expoPushToken));  // Dispatch the action to update the token
      }
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    setExpoPushToken: (state, action) => {
      console.log("Updated expoPushToken:", action.payload);
      state.expoPushToken = action.payload;
    },
  },
  extraReducers: (builder) => {
